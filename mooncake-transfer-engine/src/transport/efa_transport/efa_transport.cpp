...
EfaTransport::EfaTransport() {
    LOG(INFO) << "[EFA] Initializing EFA transport using libfabric";
}
...
int EfaTransport::registerLocalMemoryInternal(void *addr, size_t length,
    ...
    // EFA uses FI_MR_LOCAL | FI_READ | FI_WRITE | FI_REMOTE_READ | FI_REMOTE_WRITE
    const uint64_t kAccessRights = FI_READ | FI_WRITE | FI_REMOTE_READ | FI_REMOTE_WRITE;
    ...
    if (use_parallel_reg) {
        ...
        for (size_t i = 0; i < context_list_.size(); ++i) {
            reg_threads.emplace_back([this, &ret_codes, i, addr, length, kAccessRights]() {
                ret_codes[i] =
                    context_list_[i]->registerMemoryRegion(addr, length, kAccessRights);
            });
        }
...
int EfaTransport::allocateLocalSegmentID() {
    ...
    for (auto &entry : context_list_) {
        ...
        device_desc.lid = 0;  // EFA doesn't use LID
        device_desc.gid = entry->getDeviceAddress();
    ...
    for (size_t index = 0; index < task_list.size(); ++index) {
        ...
        for (uint64_t offset = 0; offset < request.length;
            ...
            if (nr_slices >= kSubmitWatermark) {
                for (auto &entry : slices_to_post){
			LOG(WARNING) <<"calling submitPostSend";
...
int EfaTransport::initializeEfaResources() {
    ...
    if (local_topology_->empty()) {
        LOG(ERROR) << "EfaTransport: No available EFA device";
