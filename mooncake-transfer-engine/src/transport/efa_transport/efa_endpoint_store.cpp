...
namespace mooncake {
std::shared_ptr<EfaEndPoint> FIFOEfaEndpointStore::getEndpoint(
...
std::shared_ptr<EfaEndPoint> FIFOEfaEndpointStore::insertEndpoint(
    const std::string &peer_nic_path, EfaContext *context) {
    ...
    auto endpoint = std::make_shared<EfaEndPoint>(*context);
    if (!endpoint) {
...
int FIFOEfaEndpointStore::deleteEndpoint(const std::string &peer_nic_path) {
    RWSpinlock::WriteGuard guard(endpoint_map_lock_);
    auto iter = endpoint_map_.find(peer_nic_path);
...
void FIFOEfaEndpointStore::evictEndpoint() {
...
void FIFOEfaEndpointStore::reclaimEndpoint() {
    RWSpinlock::WriteGuard guard(endpoint_map_lock_);
    std::vector<std::shared_ptr<EfaEndPoint>> to_delete;
...
size_t FIFOEfaEndpointStore::getSize() { return endpoint_map_.size(); }

int FIFOEfaEndpointStore::destroyQPs() {
    return 0;
}

int FIFOEfaEndpointStore::disconnectQPs() {
    return 0;
}

size_t FIFOEfaEndpointStore::getTotalQPNumber() {
    RWSpinlock::ReadGuard guard(endpoint_map_lock_);
    size_t total_qps = 0;
...
std::shared_ptr<EfaEndPoint> SIEVEEfaEndpointStore::getEndpoint(
    const std::string &peer_nic_path) {
    RWSpinlock::ReadGuard guard(endpoint_map_lock_);
    auto iter = endpoint_map_.find(peer_nic_path);
    if (iter != endpoint_map_.end()) {
...
std::shared_ptr<EfaEndPoint> SIEVEEfaEndpointStore::insertEndpoint(
    const std::string &peer_nic_path, EfaContext *context) {
    RWSpinlock::WriteGuard guard(endpoint_map_lock_);
    if (endpoint_map_.find(peer_nic_path) != endpoint_map_.end()) {
        LOG(WARNING) << "Endpoint " << peer_nic_path
                  << " already exists in SIEVEEndpointStore";
        return endpoint_map_[peer_nic_path].first;
    }
    auto endpoint = std::make_shared<EfaEndPoint>(*context);
    if (!endpoint) {
        LOG(ERROR) << "Failed to allocate memory for EfaEndPoint";
        return nullptr;
    }
    auto &config = globalConfig();
    int ret = endpoint->construct(context->cq(), config.num_qp_per_ep, config.max_sge,
                            config.max_wr, config.max_inline);
    if (ret) return nullptr;
...
int SIEVEEfaEndpointStore::deleteEndpoint(const std::string &peer_nic_path) {
    RWSpinlock::WriteGuard guard(endpoint_map_lock_);
    auto iter = endpoint_map_.find(peer_nic_path);
    // remove endpoint but leaving it status unchanged
    // in case it is setting up connection or submitting slice
...
void SIEVEEfaEndpointStore::evictEndpoint() {
    if (fifo_list_.empty()) {
        return;
    }
...
void SIEVEEfaEndpointStore::reclaimEndpoint() {
...
int SIEVEEfaEndpointStore::destroyQPs() {
    return 0;
}

int SIEVEEfaEndpointStore::disconnectQPs() {
    return 0;
}

size_t SIEVEEfaEndpointStore::getSize() { return endpoint_map_.size(); }

size_t SIEVEEfaEndpointStore::getTotalQPNumber() {
    RWSpinlock::ReadGuard guard(endpoint_map_lock_);
    size_t total_qps = 0;
